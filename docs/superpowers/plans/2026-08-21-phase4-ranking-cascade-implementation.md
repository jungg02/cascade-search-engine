...
"
```
Expected: prints a training-example count (some multiple of up to 5 per query, fewer for queries whose positive doc isn't in this 50-query slice's WAND candidates — it always is, since the positive is drawn from the run itself), then `training completed without error`. This exercises the real full-scale code path (Task 2's real `data/rank-dense-scores.jsonl`, the real dev WAND run, the real dev qrels) at a small enough slice to run in seconds — the actual Kaggle run in Task 8 is this same code, unmodified, over all 6,980 dev queries.
